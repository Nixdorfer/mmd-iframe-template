class ModelLoader {
	...
	createMissing(id, hitbox) {
		if (this.models.has(id)) return this.models.get(id)
		const boxes = hitbox?.boxes || [{ x: -0.5, y: -0.5, z: 0, w: 1, h: 1, d: 1 }]
		const vertices = []
		const faces = []
		let vIdx = 0
		for (const box of boxes) {
			const x0 = box.x, y0 = box.y, z0 = box.z
			const x1 = x0 + box.w, y1 = y0 + box.d, z1 = z0 + box.h
			vertices.push(
				[x0, y0, z0], [x1, y0, z0], [x1, y1, z0], [x0, y1, z0],
				[x0, y0, z1], [x1, y0, z1], [x1, y1, z1], [x0, y1, z1]
			)
			faces.push(
				{ v: [vIdx+0, vIdx+1, vIdx+2, vIdx+3], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] },
				{ v: [vIdx+4, vIdx+7, vIdx+6, vIdx+5], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] },
				{ v: [vIdx+0, vIdx+4, vIdx+5, vIdx+1], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] },
				{ v: [vIdx+2, vIdx+6, vIdx+7, vIdx+3], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] },
				{ v: [vIdx+0, vIdx+3, vIdx+7, vIdx+4], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] },
				{ v: [vIdx+1, vIdx+5, vIdx+6, vIdx+2], color: '#ffffff', uv: [[0,0],[1,0],[1,1],[0,1]] }
			)
			vIdx += 8
		}
		this.textureManager.createMissing(id + '_tex')
		const modelData = { name: id, origin: [0, 0, 0], scale: 1, vertices, faces }
		const model = new GLModel(this.gl, modelData, id + '_tex')
		this.models.set(id, model)
		return model
	}
