...
vec3 calRealistic(vec3 nom, vec3 viewDir, vec3 sunDir, float diff, float shadow) {
	...
	float spec = pow(max(dot(nom, halfDir), 0.0), uSpecPower);
	float ao = vAO * uAoStr;
	vec3 sunLight = uSunClr * (diff * ao + spec * 0.3) * (1.0 - shadow);
	vec3 pntLight = calPntLits(nom, viewDir);
	vec3 sptLight = calSptLits(nom, viewDir);
	return uAmbient * ao + sunLight + pntLight + sptLight;
}
...
void main() {
	...
	vec3 texClr = uUseTex > 0.5 ? texture(uTex, vUV).rgb : vec3(1.0);
	vec3 clr = vClr * texClr * light;
	fragColor = vec4(clr, 1.0);
}
`

export const TERRAIN_VERTEX_SIZE = 12
export const TERRAIN_STRIDE = TERRAIN_VERTEX_SIZE * 4

export function terrainVertexLayout(gl: WebGL2RenderingContext) {
	return [
		...
		{ name: 'aAO', size: 1, type: gl.FLOAT, normalized: false, offset: 44 }
	]
}
