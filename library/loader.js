...
if(params[0]==='EDIT'){L.edit=true;L.char=params[1]||'';L.user=params[2]||'';L.b64=params[3]||''}
else{L.edit=false;L.char=params[0]||'';L.user=params[1]||'';L.b64=params[2]||''}
...
if(!proj){document.body.innerHTML='<div style="padding:16px;background:#8b0000;border-radius:8px;color:#fff">未找到: '+L.project+'</div>';return}
fetch(L.base+proj.index+'?t='+Date.now(),{cache:'no-store'}).then(function(r){return r.text()}).then(function(h){
var cfg='<script>window.L='+JSON.stringify({loaded:{},base:L.base,edit:L.edit,project:L.project,char:L.char,user:L.user,b64:L.b64,res:L.res,platform:L.platform})+';L.load=function(libs,cb){var self=this,pending=libs.filter(function(n){return!self.loaded[n]});if(!pending.length)return cb&&cb();var c=pending.length;pending.forEach(function(n){self.loaded[n]=1;var s=document.createElement(\"script\");s.src=self.base+\"library/\"+n+\".js?t=\"+Date.now();s.onload=function(){if(--c===0&&cb)cb()};document.head.appendChild(s)})};<\/script>';
document.open();document.write(cfg+h);document.close();
